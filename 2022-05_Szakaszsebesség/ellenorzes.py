...
def time2msec( hour, minute, sec, msec):
    """ Milliszekundumban adja meg az időpontot (napon belül)

    hour, minute, sec, msec: str
    Return: int
    """
    return int(hour)*3600*1000 + int(minute)*60_000 + int(sec)*1000 + int(msec)


def sebesség( t1, t2):
    """ Kiszámolja az átlagsebességet a 10km-es szakaszon

    t1, t2: a belépési és kilépési időpontok milliszekundumban, int
    Return: int
    """
    return  10*3600*1000/(t2 - t1)
...
with open( fn_meresek) as ff:
    ...
    for sor in ff:
        ...
        t1= time2msec( h1,m1,s1,ms1)
        t2= time2msec( h2,m2,s2,ms2)
        seb= sebesség( t1,t2)
...
t1_kért= time2msec( h, m, 0, 0)
t2_kért= time2msec( h, m, 59, 999)

h= int(h)
m= int(m)
for rsz, t1, t2, seb in lmeresek:

    if t1//(1000*3600) == h and (t1//(60*1000))%60 == m:
        kp_áthaladt += 1
